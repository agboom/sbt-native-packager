...
object JavaAppPackaging extends AutoPlugin {
  ...
  import JavaAppPackaging.autoImport.*
  ...
  override def projectSettings: Seq[Def.Setting[?]] = Def.settings(
    ...
    scriptClasspathOrdering += {
      ...
      jar -> ("lib/" + MappingsHelper.makeJarName(id.organization, id.name, id.revision, art.name, art.classifier))
    },
    ...
    scriptClasspath := {
      ...
      val ordering = scriptClasspathOrdering.value.map { case (file, s) =>
        toFile(file, extracted) -> s
      }
      makeRelativeClasspathNames(ordering)
    },
  ...
  private def dependencyProjectRefs(build: Compat.BuildDependencies, thisProject: ProjectRef): Seq[ProjectRef] =
  ...
  private def isRuntimeArtifact(dep: Attributed[CompatFile], extracted: Extracted): Boolean =
    MappingsHelper.getArtifact(dep).map(_.`type` == "jar").getOrElse {
      val name = Compat.toFile(dep.data, extracted).getName
      !(name.endsWith(".jar") || name.endsWith("-sources.jar") || name.endsWith("-javadoc.jar"))
    }

  private def findProjectDependencyArtifacts: Def.Initialize[Task[Def.Classpath]] =
    Def.task {
      ...
      artTasks
        .fold[Task[Def.Classpath]](task(Nil)) { (previous, next) =>
          for {
            p <- previous
            n <- next
          } yield p ++ n.filter(isRuntimeArtifact(_, extracted))
        }
        .value
    }

  private def extractArtifacts(stateTask: Task[State], ref: ProjectRef): Task[Def.Classpath] =
    stateTask.flatMap { state =>
      ...
      } yield MappingsHelper.setArtifact(art, Attributed.blank(file))
    }

  private def findRealDep(
    dep: Attributed[CompatFile],
    projectArts: Seq[Attributed[CompatFile]],
    extracted: Extracted
  ): Option[Attributed[CompatFile]] =
    if (toFile(dep.data, extracted).isFile) Some(dep)
    else
      projectArts.find { art =>
        ...
        (MappingsHelper.getArtifact(art), MappingsHelper.getArtifact(dep)) match {
          case (Some(l), Some(r)) =>
  ...
  private def universalDepMappings(
    deps: Def.Classpath,
    projectArts: Def.Classpath,
    extracted: Extracted
  ): Seq[(CompatFile, String)] =
    for {
      dep <- deps
      realDep <- findRealDep(dep, projectArts, extracted)
    } yield realDep.data -> ("lib/" + MappingsHelper.getJarFullFilename(realDep, extracted))
}
